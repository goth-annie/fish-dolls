...
3. [実用的な使い方例](#3-実用的な使い方例)  
   ...
   3.7 [非同期処理の導入（UniTask/await対応）](#37-非同期処理の導入（unitaskawait-対応）)
...
しかし、これらは実際のゲームには不要であり、**そのままにしておくと最終ビルドにも含まれてしまう可能性があります**。

`Samples` フォルダごと削除して、本番環境に不要なデータを含めないようにしましょう。
